  ...
  TokenErrorReporter
{
  ...
  function mintInternal(uint mintAmount) internal nonReentrant {
    ...
    if (msg.sender == handler) {
      console2.log('msg.sender == handler');
      return mintFreshForUser(payable(tx.origin), mintAmount);
    }
   ...
   * @param mintAmount The amount of the underlying asset to supply
   */
  function mintFreshForUser(address minter, uint mintAmount) internal {
    require(msg.sender == handler, "Only handler can mintForUser");
    ...
    uint actualMintAmount = doTransferIn(msg.sender, mintAmount); //transfer from handler
    uint mintTokens = div_(actualMintAmount, exchangeRate);
    totalSupply = totalSupply + mintTokens;
    accountTokens[minter] = accountTokens[minter] + mintTokens; //apply changes to requester
  ...
  function borrowInternal(uint borrowAmount) internal nonReentrant {
    ...
    if (msg.sender == handler) {
      console2.log('msg.sender == handler');
      return borrowFreshForUser(payable(tx.origin), borrowAmount);
    }
    // borrowFresh emits borrow-specific logs on errors, so we don't need to
    borrowFresh(payable(msg.sender), borrowAmount);
  }

  function borrowFreshForUser(address payable borrowOrigin, uint borrowAmount) internal {
    require(msg.sender == handler, "Only handler can borrowForUser");
    ...
    uint accountBorrowsPrev = borrowBalanceStoredInternal(borrowOrigin);
    uint accountBorrowsNew = accountBorrowsPrev + borrowAmount;
    uint totalBorrowsNew = totalBorrows + borrowAmount;

    accountBorrows[borrowOrigin].principal = accountBorrowsNew;
    accountBorrows[borrowOrigin].interestIndex = borrowIndex;
    totalBorrows = totalBorrowsNew;
