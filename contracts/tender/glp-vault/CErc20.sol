...
contract CErc20 is CToken, CErc20Interface {
  ...
  function mint(uint mintAmount) external override returns (uint) {
    ...
    if(msg.sender == handler){
      comptroller.addToMarketExternal(address(this), tx.origin);
    } else {
  ...
  function compound() external override returns (uint) {
    compoundInternal();
    return NO_ERROR;
  }
   ...
   * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
   */
  function redeemUnderlyingForUser(
    uint redeemAmount,
    address user
  ) external override returns (uint) {
  ...
  function borrow(uint borrowAmount) external override returns (uint) {
    console2.log('CERC20 msg.sender %s', msg.sender);
  ...
  function depositNFT(address _NFTAddress, uint256 _TokenID) external override {
