 ...
 * @author Compound
 */
contract CErc20Vault is VaultCToken, CErc20VaultInterface {
    /**
    ...
    function redeem(uint redeemTokens) override external returns (uint) {
        require(msg.sender == vault, "Only vault can redeem");
    ...
    function redeemUnderlying(uint redeemAmount) override external returns (uint) {
        require(msg.sender == vault, "Only vault can redeemUnderlying");
    ...
    function redeemUnderlyingForUser(uint redeemAmount, address user) external returns (uint) {
        require(msg.sender == vault, "Only vault can redeemUnderlyingForUser");
    ...
    function doTransferIn(address from, uint amount) virtual override internal returns (uint) {
        ...
        if(isGLP){
            require(address(stakedGLP) != address(0), 'Error: stakedGLP address not set');
