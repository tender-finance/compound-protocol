 ...
 * @author Compound
 */
contract CErc20DelegatorVault is CTokenInterface, CErc20Interface, CDelegatorInterface {
    /**
    ...
    /**
      ...
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function borrow(uint borrowAmount, uint leverageAmount) override external returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("borrow(uint256,uint256)", borrowAmount, leverageAmount));
        return abi.decode(data, (uint));
    }
